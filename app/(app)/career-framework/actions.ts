...
export interface CareerFrameworkData {
  ...
  /** Levels are managed in Settings now; this list only feeds the Add-title dialog's level picker. */
  jobGrades: JobGrade[];
}
...
// Level (job-grade scale) management moved to Settings — see
// app/(app)/settings/actions.ts (gated on settings:manage). Career Framework
// no longer manages the level scale; it only records titles.
