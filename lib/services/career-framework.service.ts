...
export async function createJobFamily(
  ...
  return withTransaction(db, async (tx) => {
    ...
    // department id must never anchor a sub-division.
  ...
  return withTransaction(db, async (tx) => {
    ...
    if (positionCount > 0) {
      throw new ConflictError(
        `This sub-division is used by ${positionCount} position(s) and cannot be deleted.`
      );
    }
...
export async function createLevelMappingEntry(
  ...
  return withTransaction(db, async (tx) => {
    ...
    if (!family) {
      throw new CrossCompanyError(
        `Sub-division ${input.jobFamilyId} does not exist in this company.`
      );
    }
    ...
    if (!input.careerTrackId) {
      ...
      if (chosen.jobFamilyId !== input.jobFamilyId) {
        throw new ConflictError("The career track does not belong to the chosen sub-division.");
      }
      track = chosen;
    }
