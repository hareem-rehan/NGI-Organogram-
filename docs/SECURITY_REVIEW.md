...
- **Known residual finding (not a defect, a documented Next.js framework behavior):** `Cache-Control: private, no-store` was set in `proxy.ts`, but Next.js's own dynamic-rendering pipeline overrides it with `no-cache, must-revalidate` for every dynamically-rendered page response, downstream of the Proxy layer — verified via live `fetch()` against both an authenticated route and `/sign-in`. Practical impact is materially similar: `no-cache, must-revalidate` still forces origin revalidation before any cache may serve a copy, addressing the same underlying risk (private data served stale from a shared cache) that `no-store` targets.
