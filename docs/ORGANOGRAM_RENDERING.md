...
2. **Opens at L7 and above.** The threshold reads the stored **job grade**, never the title — `docs/DECISIONS.md` D1/D2. A position whose title looks senior but whose stored grade is junior stays folded, and vice versa. Below the threshold, `belowThreshold: "collapse"` (the default) keeps roles in the graph and folds them behind their manager's expand control, so the chart opens at leadership level and drills all the way down; `"hide"` removes them outright and is still supported.
3. **Unfilled roles are drawn; ungraded ones are not.** A role nobody holds appears with no name line and no "Vacant" stamp — the company's own chart does exactly this, and their position list is a ladder where most rungs are empty (`docs/DECISIONS.md` D9). A position with no grade at all cannot be placed against the threshold, so it is left off and **counted**. The two are reported separately above the chart, because "folded, one click away" and "not drawn, because the data is incomplete" are different facts.
